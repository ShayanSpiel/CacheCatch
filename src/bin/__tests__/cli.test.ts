...
assert(sample.stdout.includes("RECOVERABLE CACHE LOSS"), "sample report renders")
...
assert(realDefault.includes("Current cache-read rate") && realDefault.includes("unknown"), "missing cache telemetry shows cache-read as unknown")
assert(!realDefault.includes("FULL AGENT REPAIR PROMPT"), "default real output hides full agent prompt")
assert(!realDefault.includes("zero_value.route"), "$0 routes are hidden in default output")
assert(realFull.includes("zero_value.route") && realFull.includes("$0"), "$0 routes are visible in --full output")
assert(realDefault.includes(`${longRoute}:`) && realDefault.includes("moves later than token"), "validation plan uses actual audited route names")
assert(!realDefault.includes("* delta ="), "rendered math never falls back to broken generic delta formula")

const formulaMatch = realDefault.match(/([0-9,.]+)M \* \$([0-9.]+) \/ 1M = \$([0-9,.]+)/)
if (formulaMatch) {
  const tokensM = Number(formulaMatch[1].replace(/,/g, ""))
  const delta = Number(formulaMatch[2])
  const shown = Number(formulaMatch[3].replace(/,/g, ""))
  assert(Math.abs(tokensM * delta - shown) < 1, "displayed math inputs produce displayed dollar output")
} else {
  assert(false, "rendered math includes parseable dollar formula")
}
